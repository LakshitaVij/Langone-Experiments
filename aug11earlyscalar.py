...
class ResNetBranchEarlyFrozen(ResNetBranch):
    def __init__(self, block, layers, in_chans, clinical_ckpt=None):
        super().__init__(block, layers, in_chans)
        self.clinical_encoder = FrozenClinicalEncoder(clinical_ckpt=clinical_ckpt)
        # MLP: 64 clinical embedding -> 1 attention scalar per spatial location
    ...
    def forward(self, x, tabular_features):
        ...
        _, _, D, H, W = out.shape
        attn = self.spatial_attn(clinical_emb)
        attn = torch.sigmoid(attn)
        attn = attn.view(attn.size(0), 1, 1, 1, -1)
        attn = torch.nn.functional.interpolate(attn, size=(D, H, W), mode='nearest')
        out = out * attn
        out = out[:, :, :13, :, :]
...
class TriSeriesModelFrozen(Base3DResNet):
    DWI_KEYS = ("adc", "b1500")
        ...
        for key in self.series:
            if self.stack_adc_b1500 and key in self.DWI_KEYS:
                if not self._dwi_branch_added:
                    ...
                    self.branches[branch_name] = ResNetBranchEarlyFrozen(
            ...
            branch_name = key
            self.branches[branch_name] = ResNetBranchEarlyFrozen(
                Bottleneck, [3, 4, 6, 3], 1, clinical_ckpt=clinical_ckpt
            )
    ...
    def on_train_start(self):
        print("Freezing ResNet branches and clinical encoder (permanent)...")
        for name, param in self.named_parameters():
            if "clinical_encoder" in name:
                param.requires_grad = False
            elif not name.startswith("fc") and not name.startswith("branches") or \
