...
class ResNetBranchEarly(ResNetBranch):
    def __init__(self, block, layers, in_chans, tabular_dim=37):
        ...
        # MLP: 37 clinical numbers → 1 attention scalar per spatial location
    ...
    def forward(self, x, tabular_features):
        ...
        # clinical features create spatial attention map to guide ResNet features
        out = F.pad(out, (0, 0, 0, 0, 0, 3))
        attn = self.spatial_attn(tabular_features)
        attn = attn.view(attn.size(0), 4,5,5)
        attn = torch.sigmoid(attn)
        attn = torch.repeat_interleave(attn, repeats=4, dim=1)  # (batch, 16, 5, 5)
        attn = torch.repeat_interleave(attn, repeats=9, dim=2)  # (batch, 16, 45, 5)
        attn = torch.repeat_interleave(attn, repeats=9, dim=3)  # (batch, 16, 45, 45)
        attn = attn.unsqueeze(1)  # (batch, 1, 16, 45, 45)
        out = out * attn
        out = out[:, :, :13, :, :]
...
class TriSeriesModel(Base3DResNet):
    ...
    def on_train_start(self):
        # Keep ResNet branches frozen forever — only train spatial_attn + fc
        print("Freezing ResNet branches (permanent)...")
        for name, param in self.named_parameters():
            if not name.startswith("fc") and not name.startswith("branches") or \
               ("branches" in name and "spatial_attn" not in name):
                param.requires_grad = False
        # Ensure spatial_attn and fc are trainable
        for name, param in self.named_parameters():
            if "spatial_attn" in name or name.startswith("fc"):
